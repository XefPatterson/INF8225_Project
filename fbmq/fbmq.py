...
class Page(object):
    def __init__(self, page_access_token, bot_id, bot_name, **options):
    ...
    def handle_webhook(self, payload, sender_id, next_bot_id, optin=None, message=None, echo=None, delivery=None,
        ...
        for event in get_events(data):
            if event.is_optin:
                self._call_handler('optin', optin, event, sender_id, next_bot_id)
            elif event.is_echo:
                self._call_handler('echo', echo, event, sender_id, next_bot_id)
            elif event.is_quick_reply:
                event.matched_callbacks = self.get_quick_reply_callbacks(event)
                self._call_handler('message', message, event, sender_id, next_bot_id)
                ...
                self._call_handler('delivery', delivery, event, sender_id, next_bot_id)
            elif event.is_postback:
                event.matched_callbacks = self.get_postback_callbacks(event)
                self._call_handler('postback', postback, event, sender_id, next_bot_id)
                for callback in event.matched_callbacks:
                    callback(event.postback_payload, event)
            elif event.is_read:
                self._call_handler('read', read, event, sender_id, next_bot_id)
            elif event.is_account_linking:
                self._call_handler('account_linking', account_linking, event, sender_id, next_bot_id)
            else:
    ...
    def add_bot(self, bot_id, bot_name):
        print("Add a new bot to the conversation: \nID: {}\nName: {}".format(bot_id, bot_name))
        self.all_bots[bot_id] = bot_name
    ...
    def get_user_identity(self, data):
        try:
            ...
            if next_bot_id == MessageType.HUMAN_TURN:
                ...
                # Human has been notified
                return bot_id, next_bot_id

            if bot_id not in self.all_bots:
