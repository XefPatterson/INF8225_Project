...
def embedding_attention_seq2seq(encoder_inputs,
    ...
    with variable_scope.variable_scope(
        ...
        if isinstance(feed_previous, bool):
            raise AttributeError("feed_previous is a boolean, but it should be a tensor")
        ...
        def decoder(feed_previous_bool):
            ...
            with variable_scope.variable_scope(
                ...
                if nest.is_sequence(state):
                    state_list = nest.flatten(state)
                return outputs + state_list, attentions

        output_and_state_test, test_attention = decoder(True)
        output_and_state_train, train_attention = decoder(False)

        attentions = control_flow_ops.cond(feed_previous, lambda: test_attention, lambda: train_attention)
        outputs_and_state = control_flow_ops.cond(feed_previous, lambda: output_and_state_test,
                                                  lambda: output_and_state_train)
        ...
        if nest.is_sequence(encoder_state):
            state = nest.pack_sequence_as(
                structure=encoder_state, flat_sequence=state_list)
        return outputs_and_state[:outputs_len], state, attentions
    ...
    with ops.name_scope(name, "model_with_buckets", all_inputs):
        for j, bucket in enumerate(buckets):
            with variable_scope.variable_scope(
                ...
                bucket_infos = seq2seq(encoder_inputs[:bucket[0]],
                                       decoder_inputs[:bucket[1]])
                outputs.append(bucket_infos[0])
                if save_attention:
                    attentions.append(bucket_infos[2])
